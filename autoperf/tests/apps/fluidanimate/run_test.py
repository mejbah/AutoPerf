...
# python run_test.py [outputsNUMBER(eventSet number)]
...
for line in f.readlines():
  ...
  if len(line)>0:
	...
	nthreadList.append(line.split()[1])
	numberOfTest += 1
	

assert len(inputList) == len(nthreadList)

for i in range(numberOfTest): #TODO: frame number param change with input file name
  TEST_ARGS = nthreadList[i] + " 500 " +  DATASET_HOME+"/"+ APP_NAME + "/" + inputList[i]+" "+ "out.fluid"
